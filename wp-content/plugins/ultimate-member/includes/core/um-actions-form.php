...
/**function um_custom_validate_username( $args ) {
	...
	if ( isset( $args['user_login'] ) && !preg_match('/^[b-bB-B]\d{7}$/', $args['user_login']))	{
		UM()->form()->add_error( 'user_login', 'Your username must not contain the word admin.' );
	}
}
add_action('um_submit_form_errors_hook__registration','um_custom_validate_username', 999, 1);
*/
...
function um_custom_validate_username( $key, $array, $args ) {
	if ( isset( $args[$key] ) && !preg_match('/^[b-bB-B]\d{7}$/', $args[$key]) ) {
		UM()->form()->add_error( $key, __( 'Please enter valid username.', 'ultimate-member' ) );
	}
}
add_action( 'um_custom_field_validation_username', 'um_custom_validate_username', 30, 3 );
